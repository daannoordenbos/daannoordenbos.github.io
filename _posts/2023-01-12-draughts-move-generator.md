...
# Bitboards
Now that we know the specific draughts variant we are looking at. Yes, there are other variant with different rules. We will introduce the concept of bitboards and basic bit operations. So, what are bitboard? A bitboard is a bit array, where each bit corresponds to a game board space or piece. So we can use bitboards to store binary information about the squares of a game board. Bitboards are (generally) not suitable for games which have more than 64 squares or strange geometries. Games like Go, Stratego or Abalone. Bitboards are especially useful in chess, but they can also be made to work in draughts. The main advantage of bitboards is that they allow for parallel operations on all squares of the board. Most commonly these operations are: AND, OR, XOR, NOT and shift. Moreover, there are operations that find the index of the most/lest significant bits, get the most/least significant bit and even count the number of bits ber of of bits of a bitboard quickly.
...
Notice that not all bits are used. Partly because a 64 bit integer has redundancy when only needing to store 50 bits. More importantly however, we see that the bits on positions 10, 21, 32, 43 are missing. This perculiar exclusion ensures that the board has nicer properties. The diagonals can be traversed by adding or subtracting 5 or 6 succesivly. Moreover, this bitmapping also handles edge detection. If these so called 'ghost' squared would not be included then the bitboard mapping would not have these nice properties. Why this is so useful will become more clear when we move on to the move generation. 
